...
if [ -e /Library/Bitdefender/AVP/EndpointSecurityforMac.app ]; then
  if pkgutil --check-signature "/Library/Bitdefender/AVP/EndpointSecurityforMac.app" | tr -d '[:space:]' | grep -q "$bitdefender_cert_installed"; then
...
if [ "$arch_name" = "x86_64" ]; then
    # "Running on Intel"
    # FILL IN company_code based on the info from Gravity Zone
    company_code="xxxxx"
    dl_link="https://cloudgz.gravityzone.bitdefender.com/Packages/MAC/0/$company_code/Bitdefender_for_MAC.dmg"
    pkg_name="antivirus_for_mac.pkg"
elif [[ "$arch_name" =~ "arm" ]]; then
    # "Running on ARM"
    dl_link="https://cloudgz.gravityzone.bitdefender.com/Packages/MAC/0/$company_code/Bitdefender_for_MAC_ARM.dmg"
